...
export function useGridottoCoreV2() {
  ...
  const bulkBuyForSelectedFollowers = async (selectedAddresses: string[]) => {
    if (!contract || !account || !web3) {
      throw new Error('Wallet not connected');
    }
    ...
    try {
      ...
      console.log('[bulkBuyForSelectedFollowers] Starting with addresses:', selectedAddresses);
      
      // Get current ticket price
      const ticketPrice = await contract.methods.ticketPrice().call();
      console.log('[bulkBuyForSelectedFollowers] Ticket price:', ticketPrice);
      
      // Calculate total cost
      const validAddresses = selectedAddresses.filter(addr => addr && addr !== '0x0000000000000000000000000000000000000000' && addr !== account);
      const totalCost = BigInt(ticketPrice) * BigInt(validAddresses.length);
      
      console.log('[bulkBuyForSelectedFollowers] Valid addresses count:', validAddresses.length);
      console.log('[bulkBuyForSelectedFollowers] Total cost:', totalCost.toString());
      console.log('[bulkBuyForSelectedFollowers] Total cost in ETH:', web3.utils.fromWei(totalCost.toString(), 'ether'));
      ...
      console.log('[bulkBuyForSelectedFollowers] Transaction completed:', tx);
      return tx;
    } catch (err: any) {
      console.error('[bulkBuyForSelectedFollowers] Error occurred:', err);
      setError(err.message);
      throw err;
    } finally {
      setLoading(false);
  ...
  // Fund weekly pool (admin only)
  const fundWeeklyPool = async (amount: string) => {
    if (!contract || !account || !web3) {
      throw new Error('Wallet not connected');
    }
    ...
    try {
      ...
      console.log('[fundWeeklyPool] Funding with amount:', amount, 'ETH');
      console.log('[fundWeeklyPool] Value in Wei:', valueInWei);
      ...
      console.log('[fundWeeklyPool] Transaction completed:', tx);
      return tx;
    } catch (err: any) {
      console.error('[fundWeeklyPool] Error occurred:', err);
      setError(err.message);
      throw err;
    } finally {
      setLoading(false);
  ...
  // Fund monthly pool (admin only)
  const fundMonthlyPool = async (amount: string) => {
    if (!contract || !account || !web3) {
      throw new Error('Wallet not connected');
    }
    ...
    try {
      ...
      const valueInWei = web3.utils.toWei(amount, 'ether');
      console.log('[fundMonthlyPool] Funding with amount:', amount, 'ETH');
      console.log('[fundMonthlyPool] Value in Wei:', valueInWei);
      ...
      console.log('[fundMonthlyPool] Transaction completed:', tx);
      return tx;
    } catch (err: any) {
      console.error('[fundMonthlyPool] Error occurred:', err);
      setError(err.message);
      throw err;
    } finally {
      setLoading(false);
