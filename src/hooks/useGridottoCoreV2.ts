...
import { NEW_DIAMOND_ABI } from '@/abi/newDiamondAbi';
...
export function useGridottoCoreV2() {
  ...
  useEffect(() => {
    ...
    if (web3) {
      ...
      const coreContract = new web3.eth.Contract(NEW_DIAMOND_ABI as any, DIAMOND_ADDRESS);
      setContract(coreContract);
      console.log('[useGridottoCoreV2] Contract instance created with NEW_DIAMOND_ABI (includes getDrawNFTDetails)');
    } else {
  ...
  const getDrawDetails = useCallback(async (drawId: number): Promise<DrawDetails | null> => {
    ...
    if (!activeContract) {
      ...
      activeContract = new web3.eth.Contract(NEW_DIAMOND_ABI as any, DIAMOND_ADDRESS);
    }
