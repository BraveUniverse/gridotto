...
export function useGridottoCoreV2() {
  ...
  useEffect(() => {
    ...
    if (web3) {
      ...
      const coreContract = new web3.eth.Contract(COMPLETE_DIAMOND_ABI as any, DIAMOND_ADDRESS);
      setContract(coreContract);
      console.log('[useGridottoCoreV2] Contract instance created with COMPLETE_DIAMOND_ABI (includes getDrawNFTDetails & getNextDrawId)');
    } else {
  ...
  const getDrawDetails = useCallback(async (drawId: number): Promise<DrawDetails | null> => {
    ...
    if (!activeContract) {
      ...
      activeContract = new web3.eth.Contract(COMPLETE_DIAMOND_ABI as any, DIAMOND_ADDRESS);
    }
