...
export const useGridottoContract = () => {
  ...
  // Buy ticket - LUKSO dökümanına göre güncellendi
  const buyTicket = useCallback(async (contextProfile: string, amount: number, randomSeed: number) => {
    ...
    try {
      ...
      const result = await contract.methods.buyTicket(contextProfile, amount, randomSeed).send({
        from: account,
        value: totalPrice
      });
